...
public class FuncitonalPrgrammin02 {
    ...
    public static void printElFunctional(List<Integer> l) {
        l.stream().forEach(Utils::printInTheSameLineWithASpace); //12 9 13 4 9 2 4 12 15
    }
    ...
    public static void PrintEvensFunctional(List<Integer> l) {
        l.stream().filter(Utils ::checkToBeEven).forEach(Utils::printInTheSameLineWithASpace); //12 4 2 4 12
    }
    ...
    public static void printSquaresOfOdd (List<Integer> l) {
        ...
        l.stream().filter(Utils ::checkToBeOdd).map(Utils::getSqure).forEach(Utils::printInTheSameLineWithASpace); //81 169 81 225
    ...
    public static void printCubeOfDistinctOdds (List<Integer> l) {
        l.stream().distinct().filter(Utils::checkToBeOdd).map(Utils::getCube).forEach(Utils::printInTheSameLineWithASpace); //729 2197 3375
    }
    ...
    public static void sumOfSquareDistinctEvens (List<Integer> l) {
        ...
        Integer sum = l.stream().distinct().filter(Utils::checkToBeEven).map(Utils::getSqure).reduce(0, Math::addExact);
